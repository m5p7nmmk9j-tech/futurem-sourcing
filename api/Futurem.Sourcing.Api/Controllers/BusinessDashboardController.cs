...
public class BusinessDashboardController : ControllerBase
{
    ...
    public async Task<ActionResult<object>> Summary()
    {
        ...
        return new
        {
            ...
            today = new
            {
                rfqs = await _db.Rfqs.CountAsync(x => x.CreatedAt.Date == today),
                customerOrders = await _db.CustomerOrders.CountAsync(x => x.CreatedAt.Date == today),
                purchaseOrders = po.Count(x => x.CreatedAt.Date == today),
                summaryOrders = so.Count(x => x.CreatedAt.Date == today),
                receivingOrders = await _db.ReceivingOrders.CountAsync(x => x.CreatedAt.Date == today),
                qcOrders = qc.Count(x => x.CreatedAt.Date == today),
                containerLoads = containers.Count(x => x.CreatedAt.Date == today),
                shipments = shipments.Count(x => x.CreatedAt.Date == today),
                received = payments.Where(x => x.Direction == "receive" && x.PaymentDate.Date == today).Sum(x => x.Amount),
                paid = payments.Where(x => x.Direction == "pay" && x.PaymentDate.Date == today).Sum(x => x.Amount),
                profit = IncomeAmount(today, today) - CostAmount(today, today)
            },
            month = new
            {
                sales = so.Where(x => x.OrderDate.HasValue && x.OrderDate.Value.Date >= monthStart).Sum(x => x.ReceivableAmount),
                purchase = payables.Where(x => (x.RecordDate ?? x.CreatedAt).Date >= monthStart && x.TargetType == "PO").Sum(x => x.Amount),
                received = payments.Where(x => x.Direction == "receive" && x.PaymentDate.Date >= monthStart).Sum(x => x.Amount),
                paid = payments.Where(x => x.Direction == "pay" && x.PaymentDate.Date >= monthStart).Sum(x => x.Amount),
                profit = IncomeAmount(monthStart, today) - CostAmount(monthStart, today)
            },
            finance = new
            {
