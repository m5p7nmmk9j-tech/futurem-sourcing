...
public class GlobalSearchController : ControllerBase
{
    ...
    public async Task<ActionResult<object>> Search([FromQuery] string keyword, [FromQuery] int top = 10)
    {
        ...
        items.AddRange(await _db.Customers.Where(x => EF.Functions.Like(x.Code, like) || EF.Functions.Like(x.Name, like) || EF.Functions.Like(x.Contact, like) || EF.Functions.Like(x.Email, like)).OrderByDescending(x => x.Id).Take(top).Select(x => new { module = "customers", type = "客户", id = x.Id, no = x.Code, title = x.Name, route = "/customers" }).ToListAsync());
        items.AddRange(await _db.Suppliers.Where(x => EF.Functions.Like(x.Code, like) || EF.Functions.Like(x.Name, like) || EF.Functions.Like(x.Contact, like) || EF.Functions.Like(x.Email, like)).OrderByDescending(x => x.Id).Take(top).Select(x => new { module = "suppliers", type = "供应商", id = x.Id, no = x.Code, title = x.Name, route = "/suppliers" }).ToListAsync());
        ...
        items.AddRange(await _db.Shipments.Where(x => EF.Functions.Like(x.No, like) || EF.Functions.Like(x.Carrier, like) || EF.Functions.Like(x.BlNo, like) || EF.Functions.Like(x.Status, like)).OrderByDescending(x => x.Id).Take(top).Select(x => new { module = "shipments", type = "出运", id = x.Id, no = x.No, title = x.Carrier + " " + x.BlNo, route = "/shipments" }).ToListAsync());
