  ...
  <div class="page">
    ...
    <div class="card">
      ...
      <el-table :data="rows" border stripe>
        ...
        <el-table-column label="操作" width="560" fixed="right"><template #default="scope"><el-button size="small" type="success" @click="selectRow(scope.row)">明细</el-button><el-button size="small" type="warning" @click="loadUtilization(scope.row)">容量预警</el-button><el-button size="small" type="primary" @click="recommendByContainer(scope.row)">推荐柜型</el-button><el-button size="small" type="primary" @click="openShipmentDialog(scope.row)">生成出运</el-button><el-button size="small" @click="openEdit(scope.row)">编辑</el-button><el-button size="small" @click="copy(scope.row.id)">复制</el-button><el-button size="small" type="danger" @click="remove(scope.row.id)">删除</el-button></template></el-table-column>
      </el-table>
      ...
      <el-row v-if="utilization.message" :gutter="12" style="margin-top:12px">
        <el-col :span="3"><el-statistic title="柜型" :value="utilization.containerType || '-'" /></el-col>
        <el-col :span="3"><el-statistic title="容量CBM" :value="utilization.capacityCbm || 0" /></el-col>
        <el-col :span="3"><el-statistic title="已装CBM" :value="utilization.cbm || 0" /></el-col>
        <el-col :span="3"><el-statistic title="剩余CBM" :value="utilization.remainingCbm || 0" /></el-col>
        <el-col :span="3"><el-statistic title="CBM利用率%" :value="utilization.cbmRate || 0" /></el-col>
        <el-col :span="3"><el-statistic title="重量利用率%" :value="utilization.weightRate || 0" /></el-col>
        <el-col :span="3"><el-statistic title="箱数" :value="utilization.cartons || 0" /></el-col>
        <el-col :span="3"><el-statistic title="总KG" :value="utilization.gw || 0" /></el-col>
      </el-row>
...
async function recommendByContainer(row:any){selectedId.value=row.id; const res=await http.get('/container-loads/recommend',{params:{containerLoadId:row.id}}); Object.assign(recommendation,res.data)}
