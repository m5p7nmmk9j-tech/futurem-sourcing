<template>
  <div class="page">
    <div class="page-header"><div class="page-title">QC 验货</div><el-button type="primary" @click="openCreate">新增 QC</el-button></div>
    <div class="card">
      <div class="toolbar">
        <el-select v-model="purchaseOrderId" placeholder="按PO筛选" clearable filterable style="width:280px" @change="load"><el-option v-for="p in purchaseOrders" :key="p.id" :label="p.no" :value="p.id" /></el-select>
        <el-select v-model="receivingOrderId" placeholder="按收货单筛选" clearable filterable style="width:280px" @change="load"><el-option v-for="r in receivingOrders" :key="r.id" :label="r.no" :value="r.id" /></el-select>
        <el-button @click="load">刷新</el-button>
      </div>
      <el-table :data="rows" border stripe>
        <el-table-column prop="no" label="QC单号" width="190"/><el-table-column prop="purchaseOrderId" label="PO ID" width="100"/><el-table-column prop="receivingOrderId" label="收货单ID" width="110"/><el-table-column prop="qcDate" label="验货日期" width="150"/><el-table-column prop="result" label="结果" width="120"/><el-table-column prop="status" label="状态" width="110"/><el-table-column prop="remark" label="备注" min-width="220"/>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑 QC' : '新增 QC'" width="620px">
      <el-alert title="QC 记录验货结果，明细可记录每个商品验货数量、箱数、CBM、重量。" type="info" show-icon style="margin-bottom:12px" />
      <el-form label-width="110px"><el-form-item label="采购订单"><el-select v-model="form.purchaseOrderId" filterable clearable placeholder="选择 PO" style="width:100%"><el-option v-for="p in purchaseOrders" :key="p.id" :label="p.no" :value="p.id" /></el-select></el-form-item><el-form-item label="收货单"><el-select v-model="form.receivingOrderId" filterable clearable placeholder="选择收货单" style="width:100%"><el-option v-for="r in receivingOrders" :key="r.id" :label="r.no" :value="r.id" /></el-select></el-form-item><el-form-item label="验货日期"><el-date-picker v-model="form.qcDate" type="date" value-format="YYYY-MM-DD" style="width:100%" /></el-form-item><el-form-item label="验货结果"><el-select v-model="form.result" style="width:100%"><el-option label="待验" value="pending"/><el-option label="合格" value="passed"/><el-option label="不合格" value="failed"/><el-option label="部分合格" value="partial"/></el-select></el-form-item><el-form-item label="状态"><el-input v-model="form.status" /></el-form-item><el-form-item label="备注"><el-input v-model="form.remark" type="textarea" /></el-form-item></el-form>
...
async function save(){if(!form.purchaseOrderId&&!form.receivingOrderId)return ElMessage.warning('请选择 PO 或收货单'); const res=form.id?await http.put(`/qc-orders/${form.id}`,form):await http.post('/qc-orders',form); dialogVisible.value=false; ElMessage.success('保存成功'); await load(); selectedId.value=res.data?.id||form.id||selectedId.value}
async function remove(id:number){await ElMessageBox.confirm('确认删除该 QC？','提示'); await http.delete(`/qc-orders/${id}`); if(selectedId.value===id)selectedId.value=null; ElMessage.success('已删除'); await load()}
