<template>
  <div class="page">
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? `Edit QC: ${form.no || ''}` : 'New QC'" width="92%" destroy-on-close>
      ...
      <el-form label-width="110px">
        <el-row :gutter="16">
          <el-col :span="12"><el-form-item label="PO"><el-select v-model="form.purchaseOrderId" filterable clearable style="width:100%"><el-option v-for="p in purchaseOrders" :key="p.id" :label="p.no" :value="p.id" /></el-select></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="RCV"><el-select v-model="form.receivingOrderId" filterable clearable style="width:100%"><el-option v-for="r in receivingOrders" :key="r.id" :label="r.no" :value="r.id" /></el-select></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="Date"><el-date-picker v-model="form.qcDate" type="date" value-format="YYYY-MM-DD" style="width:100%" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="Result"><el-select v-model="form.result" style="width:100%"><el-option label="Pending" value="pending"/><el-option label="Passed" value="passed"/><el-option label="Failed" value="failed"/><el-option label="Partial" value="partial"/></el-select></el-form-item></el-col>
      ...
      <DocumentLinesEditor v-if="form.id" document-type="QC" :document-id="form.id" />
...
const purchaseOrderId=ref<number|null>(null), receivingOrderId=ref<number|null>(null), dialogVisible=ref(false)
const form=reactive<any>({id:0,no:'',purchaseOrderId:null,receivingOrderId:null,qcDate:'',status:'draft',result:'pending',remark:''})
...
async function load(){const params:any={}; if(purchaseOrderId.value)params.purchaseOrderId=purchaseOrderId.value; if(receivingOrderId.value)params.receivingOrderId=receivingOrderId.value; rows.value=(await http.get('/qc-orders',{params})).data}
function reset(){Object.assign(form,{id:0,no:'',purchaseOrderId:null,receivingOrderId:null,qcDate:'',status:'draft',result:'pending',remark:''})}
...
async function save(){if(!form.purchaseOrderId&&!form.receivingOrderId)return ElMessage.warning('PO or RCV required'); const res=form.id?await http.put(`/qc-orders/${form.id}`,form):await http.post('/qc-orders',form); if(res.data)Object.assign(form,res.data); ElMessage.success('Header saved'); await load()}
async function copy(id:number){await http.post(`/qc-orders/${id}/copy`); ElMessage.success('Copied'); await load()}
async function remove(id:number){await ElMessageBox.confirm('Delete?','Tip'); await http.delete(`/qc-orders/${id}`); ElMessage.success('Deleted'); await load()}
