  ...
  <div class="page">
    ...
    <div class="card">
      ...
      <el-table :data="rows" border stripe>
        ...
        <el-table-column label="条码图片" width="220" align="center">
          <template #default="scope">
            <BarcodeImage :value="scope.row.barcode" :height="34" :width="1.4" />
          </template>
        </el-table-column>
        <el-table-column prop="nameCn" label="中文名" min-width="180" />
        <el-table-column prop="unit" label="单位" width="80" />
        <el-table-column prop="purchasePrice" label="采购价" width="100" />
        <el-table-column prop="cartonQty" label="单箱数量" width="110" />
        <el-table-column label="尺寸cm" width="150">
          <template #default="scope">
            {{ scope.row.cartonLengthCm || 0 }}×{{ scope.row.cartonWidthCm || 0 }}×{{ scope.row.cartonHeightCm || 0 }}
          </template>
        </el-table-column>
        <el-table-column label="单箱CBM" width="110">
          <template #default="scope">{{ cartonCbm(scope.row) }}</template>
        </el-table-column>
        <el-table-column label="重量KG" width="120">
          <template #default="scope">GW {{ scope.row.cartonGwKg || 0 }} / NW {{ scope.row.cartonNwKg || 0 }}</template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑商品' : '新增商品'" width="760px">
      <el-alert
        title="SKU 创建后不允许修改；商品条码可以修改，但必须保持全系统唯一。金额、立方数、毛重和净重均保留两位小数。"
        type="info"
        show-icon
        ...
        <el-row :gutter="12">
          <el-col :span="12">
            <el-form-item label="SKU">
              <el-input v-model="form.sku" :disabled="!!form.id" placeholder="留空自动生成" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="商品条码">
              <el-input v-model="form.barcode" placeholder="留空自动生成" maxlength="80" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="条码预览">
              <BarcodeImage :value="form.barcode" :height="38" :width="1.5" />
            </el-form-item>
          </el-col>
...
const err = (error: any) => error?.response?.data?.message || error?.response?.data || error?.message || '操作失败'

async function load() {
  try {
    rows.value = (await http.get('/products', { params: { keyword: keyword.value } })).data
  } catch (error: any) {
...
function cartonCbm(row: any) {
  return Number(((Number(row.cartonLengthCm || 0) * Number(row.cartonWidthCm || 0) * Number(row.cartonHeightCm || 0)) / 1000000).toFixed(2))
}
...
async function save() {
  try {
    if (!form.nameCn) return ElMessage.warning('请输入中文名')
    if (form.id && !String(form.barcode || '').trim()) return ElMessage.warning('请输入商品条码')
    form.id ? await http.put(`/products/${form.id}`, form) : await http.post('/products', form)
...
async function remove(id: number) {
  try {
    ...
    if (error !== 'cancel' && error !== 'close') ElMessage.error(err(error))
  }
}
