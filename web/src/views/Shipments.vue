<template>
  <div class="page">
    <div class="page-header"><div><div class="page-title">出运单</div><div class="page-subtitle">一张装柜单对应一张出运单；确认资料后锁定，货柜实际离仓时才生成物流应收应付。</div></div></div>
    <div class="card">
      ...
      <el-table :data="rows" border stripe>
        ...
        <el-table-column label="状态" width="105"><template #default="s">{{shipmentStatusLabel(s.row.status)}}</template></el-table-column><el-table-column prop="etd" label="ETD" width="120"/><el-table-column label="实际离仓" width="170"><template #default="s">{{dateTime(s.row.actualDepartureAt)}}</template></el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="`出运单：${form.no||''}`" width="94%" destroy-on-close>
      <el-alert v-if="form.status==='confirmed'" title="出运资料已确认并锁定。货柜实际离开仓库后点击“确认货柜离仓发运”，系统才生成物流服务商应付并追加客户物流应收。" type="warning" show-icon style="margin-bottom:12px"/>
      <el-alert v-if="form.status==='shipped'||form.status==='completed'" :title="`已于 ${dateTime(form.actualDepartureAt)} 确认离仓，物流应收应付已生成。`" type="success" show-icon style="margin-bottom:12px"/>
      ...
      <template #footer><el-button @click="dialogVisible=false">关闭</el-button><el-button v-if="form.id&&form.status==='draft'" type="primary" @click="save">保存出运资料</el-button><el-button v-if="form.id&&form.status==='draft'" type="success" @click="confirmShipment">确认出运资料</el-button><el-button v-if="form.id&&form.status==='confirmed'" type="warning" @click="markShipped">确认货柜离仓发运</el-button></template>
    </el-dialog>
  </div>
</template>
...
const defaults={id:0,no:'',containerLoadId:null,summaryOrderId:null,shipmentMode:'SEA',currency:'RMB',carrier:'',vesselVoyage:'',billOfLadingNo:'',departurePort:'',destinationPort:'',etd:'',eta:'',actualDepartureAt:null,status:'draft',calculatedTotalCbm:0,finalTotalCbm:0,calculatedGrossWeightKg:0,finalGrossWeightKg:0,calculatedNetWeightKg:0,finalNetWeightKg:0,expenseTotal:0,customerChargeTotal:0,logisticsProfitTotal:0,remark:''}
const form=reactive<any>({...defaults}),locked=computed(()=>['confirmed','shipped','completed'].includes(form.status)),fmt=(v:any)=>Number(v||0).toFixed(2),err=(e:any)=>e?.response?.data?.message||e?.response?.data||e?.message||'操作失败'
...
function dateTime(value:any){return value?new Date(value).toLocaleString('zh-CN',{hour12:false}):''}
...
async function confirmShipment(){try{await ElMessageBox.confirm('确认后出运资料和物流费用将锁定，实际离仓前不会生成物流应收应付。是否继续？','确认出运资料');Object.assign(form,(await http.post(`/shipments/${form.id}/confirm`)).data);ElMessage.success('出运资料已确认，等待货柜离仓');await reloadCurrent()}catch(e:any){if(e!=='cancel'&&e!=='close')ElMessage.error(err(e))}}
async function markShipped(){try{await ElMessageBox.confirm('确认货柜已经实际离开仓库？确认后将生成物流服务商应付，并把客户物流费用追加到本柜客户应收单。','确认离仓发运');const response=await http.post(`/shipments/${form.id}/confirm-departure`);Object.assign(form,response.data.shipment||{});ElMessage.success('货柜已确认离仓，物流应收应付已生成');await reloadCurrent()}catch(e:any){if(e!=='cancel'&&e!=='close')ElMessage.error(err(e))}}
