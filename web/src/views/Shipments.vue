<template>
  <div class="page">
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? `Edit Shipment: ${form.no || ''}` : 'New Shipment'" width="92%" destroy-on-close>
      ...
      <el-form label-width="110px">
        <el-row :gutter="16">
          <el-col :span="12"><el-form-item label="CL"><el-select v-model="form.containerLoadId" clearable filterable style="width:100%"><el-option v-for="c in containerLoads" :key="c.id" :label="c.no" :value="c.id" /></el-select></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="SO"><el-select v-model="form.summaryOrderId" clearable filterable style="width:100%"><el-option v-for="s in summaryOrders" :key="s.id" :label="s.no" :value="s.id" /></el-select></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="Mode"><el-select v-model="form.shipmentMode" style="width:100%"><el-option label="SEA" value="SEA"/><el-option label="AIR" value="AIR"/><el-option label="EXPRESS" value="EXPRESS"/></el-select></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="Carrier"><el-input v-model="form.carrier" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="Vessel"><el-input v-model="form.vesselVoyage" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="BL"><el-input v-model="form.billOfLadingNo" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="Departure Port"><el-input v-model="form.departurePort" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="Destination Port"><el-input v-model="form.destinationPort" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="ETD"><el-date-picker v-model="form.etd" type="date" value-format="YYYY-MM-DD" style="width:100%" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="ETA"><el-date-picker v-model="form.eta" type="date" value-format="YYYY-MM-DD" style="width:100%" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="Status"><el-input v-model="form.status" /></el-form-item></el-col>
...
const form=reactive<any>({id:0,no:'',containerLoadId:null,summaryOrderId:null,shipmentMode:'SEA',carrier:'',vesselVoyage:'',billOfLadingNo:'',departurePort:'',destinationPort:'',etd:'',eta:'',status:'draft',remark:''})
...
function reset(){Object.assign(form,{id:0,no:'',containerLoadId:null,summaryOrderId:null,shipmentMode:'SEA',carrier:'',vesselVoyage:'',billOfLadingNo:'',departurePort:'',destinationPort:'',etd:'',eta:'',status:'draft',remark:''})}
...
async function save(){if(!form.shipmentMode)return ElMessage.warning('Mode required'); const res=form.id?await http.put(`/shipments/${form.id}`,form):await http.post('/shipments',form); if(res.data)Object.assign(form,res.data); ElMessage.success('Header saved'); await load()}
async function copy(id:number){await http.post(`/shipments/${id}/copy`); ElMessage.success('Copied'); await load()}
async function remove(id:number){await ElMessageBox.confirm('Delete?','Tip'); await http.delete(`/shipments/${id}`); ElMessage.success('Deleted'); await load()}
